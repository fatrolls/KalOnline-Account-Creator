...
ReTry5:
                ...
                if (response.Contains("This email domain is not allowed. Please try to sign up again.")) {
                    ...
                    using (System.IO.StreamWriter file = new System.IO.StreamWriter(System.IO.Directory.GetCurrentDirectory() + @"\Accounts.txt", true))
                    {
                        file.WriteLine("[id: " + form.txtUsername.Text + " pw: " + form.txtPassword.Text + " email: " + mail.User + "@" + mail.Host + "]");
                    }
